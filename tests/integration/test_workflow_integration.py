...
class TestWorkflowIntegration(unittest.TestCase):
    ...
    def test_workflow_login_to_report_submission(
        self, mock_form_st, mock_load_knowledge, mock_get_reports, mock_save_report, mock_get_user
    ):
        """
        Simulates:
        ...
        mock_form_st.columns.side_effect = (
            self.mock_st.columns.side_effect
        )  # Use same logic for columns
        ...
        with (
            ...
            patch("components.ui.navigation_ui.render_sidebar"),
    ...
    def test_admin_workflow_manage_shifts(
        self,
        mock_cal_st,
        mock_gt_st,
        mock_get_user,
        mock_get_shifts_view,
        mock_get_shifts_gt,
        mock_get_subs,
        mock_get_bacheca,
        mock_get_users,
        ...
        mock_get_shifts_gt.return_value = pd.DataFrame(
            columns=["ID_Turno", "Data", "Tipo", "Stato"]
        )
        ...
        mock_get_bacheca.return_value = pd.DataFrame(
            columns=[
                "Stato",
                "Data",
                "Tipo",
                "Timestamp_Pubblicazione",
                "Richiedente",
                "ID_Turno",
                "Ricevente_Matricola",
                "Motivazione",
                "Richiedente_Matricola",
                "Note",
            ]
        )
        mock_get_subs.return_value = pd.DataFrame(
            columns=[
                "Stato",
                "Data",
                "Tipo",
        ...
        def admin_columns_side_effect(spec, *args, **kwargs):
            ...
            for idx, c in enumerate(cols):
                ...
                # Bind loop variable via default arg
                def btn_se(*args, _idx=idx, **kwargs):
                    return _idx == 1
