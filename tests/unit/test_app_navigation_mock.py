...
class TestAppNavigationMock(unittest.TestCase):
    def setUp(self):
        ...
        # Auth & User
        self.patch_app_func(
            "get_user_by_matricola", return_value={"Nome Cognome": "Test User", "Ruolo": "Tecnico"}
        )

        # DB & Data
        self.patch_app_func(
            "get_all_users", return_value=[{"Nome Cognome": "Test User", "Matricola": "U1"}]
        )
        self.patch_app_func("trova_attivita", return_value=[])
        self.patch_app_func("recupera_attivita_non_rendicontate", return_value=[])
        self.patch_app_func(
            "get_validated_intervention_reports", return_value=MagicMock(empty=True)
        )
