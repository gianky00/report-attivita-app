...
def test_prenota_turno_limit_reached(mocker):
    ...
    mocker.patch("src.modules.shifts.logic_bookings.get_shift_by_id", 
                 return_value={"ID_Turno": "T1", "PostiTecnico": 1, "PostiAiutante": 0})
    
    # Mock 1 prenotazione già esistente come Tecnico
    mock_bookings = pd.DataFrame([{"Matricola": "999", "RuoloOccupato": "Tecnico"}])
    mocker.patch("src.modules.shifts.logic_bookings.get_bookings_for_shift", return_value=mock_bookings)
...
def test_pubblica_in_bacheca_success(mocker):
    ...
    mocker.patch("src.modules.shifts.logic_market.get_booking_by_user_and_shift", return_value=mock_booking)
    
    # Mock DB transaction
    mock_conn = mocker.patch("src.modules.shifts.logic_market.get_db_connection")
    mocker.patch("src.modules.shifts.logic_market.add_bacheca_item", return_value=True)
    mocker.patch("src.modules.shifts.logic_market.log_shift_change")
    mocker.patch("src.modules.shifts.logic_market.get_shift_by_id", return_value=None)
    mocker.patch("src.modules.shifts.logic_market.st")
