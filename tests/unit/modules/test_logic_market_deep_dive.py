...
class TestLogicMarketDeepDive(unittest.TestCase):
    ...
    @patch("modules.shifts.logic_market.get_user_by_matricola")
    def test_richiedi_sostituzione_success(
        self, mock_get, mock_add, mock_notif, mock_toast, mock_succ
    ):
    ...
    def test_pubblica_turno_logic_complex(
        self, mock_book, mock_conn, mock_add, mock_shift, mock_all, mock_succ
    ):
    ...
    def test_prendi_turno_success(
        self, mock_get, mock_conn, mock_upd, mock_add, mock_succ, mock_ball
    ):
        mock_get.return_value = {
            "Stato": "Disponibile",
