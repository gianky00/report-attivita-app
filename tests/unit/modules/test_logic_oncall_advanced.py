...
from modules.shifts.logic_oncall import manual_override_logic, sync_oncall_shifts


@pytest.fixture
def mock_st_oncall(mocker):
    """Mock di streamlit per i test di reperibilità."""
    return mocker.patch("modules.shifts.logic_oncall.st")
...
def test_sync_oncall_shifts_new_dates(mocker, mock_st_oncall):
    ...
    mocker.patch("modules.shifts.logic_oncall.get_all_users", return_value=mock_users)

    # Mock coppia reperibilità
    mocker.patch("modules.shifts.logic_oncall.get_on_call_pair", return_value=(("ROSSI", "Tecnico"), ("BIANCHI", "Aiutante")))

    # Mock funzioni DB
    mocker.patch("modules.shifts.logic_oncall.create_shift", return_value=True)
    mocker.patch("modules.shifts.logic_oncall.add_booking", return_value=True)
    mocker.patch("modules.shifts.logic_oncall.find_matricola_by_surname", side_effect=["M1", None])
...
def test_manual_override_logic_success(mocker, mock_st_oncall):
    ...
    mocker.patch("modules.shifts.logic_oncall.get_user_by_matricola", return_value={"Ruolo": "Tecnico"})
    mocker.patch("modules.shifts.logic_oncall.log_shift_change")
