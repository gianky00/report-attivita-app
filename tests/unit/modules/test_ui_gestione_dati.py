...
def test_render_gestione_dati_tab_success(mocker):
    ...
    mocker.patch(
        "streamlit.selectbox",
        side_effect=lambda label, options, **kwargs: options[1]
    ...
    mocker.patch(
        "pages.gestione_dati.get_all_users",
        return_value=pd.DataFrame([{"Matricola": "M1", "Nome Cognome": "T1", "Ruolo": "Tecnico"}]),
    )
    mocker.patch("pages.gestione_dati.get_all_assigned_activities", return_value=[])
    mocker.patch(
        "pages.gestione_dati.get_validated_intervention_reports", return_value=pd.DataFrame()
    )
