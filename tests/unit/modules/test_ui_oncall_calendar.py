...
def test_render_reperibilita_tab_success(mocker):
    ...
    mocker.patch(
        "streamlit.session_state", MockSessionState({"week_start_date": datetime.date.today()})
    )

    mocker.patch(
        "pages.shifts.oncall_calendar_view.get_shifts_by_type",
        return_value=pd.DataFrame(
            [{"ID_Turno": "T1", "Data": datetime.date.today().isoformat(), "Tipo": "Reperibilità"}]
        ),
    )
    mocker.patch(
        "pages.shifts.oncall_calendar_view.get_shift_by_id", return_value={"Data": "2025-01-01"}
    )
...
def test_render_reperibilita_tab_admin_edit(mocker):
    ...
    mocker.patch(
        "pages.shifts.oncall_calendar_view.get_shift_by_id",
        return_value={"Data": "2025-01-01", "ID_Turno": "T1"},
    )
    mocker.patch(
        "pages.shifts.oncall_calendar_view.get_all_users",
        return_value=pd.DataFrame([{"Matricola": "M1", "Nome Cognome": "U1"}]),
    )
