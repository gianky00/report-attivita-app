...
def test_render_ia_management_tab(mocker):
    ...
    mocker.patch(
        "pages.admin.ia_view.learning_module.load_unreviewed_knowledge",
        return_value=[
            {
                "id": "E1",
                "stato": "in attesa di revisione",
                "attivita_collegata": "A",
                "data_suggerimento": "2025-01-01T10:00:00",
                "suggerito_da": "T1",
                "pdl": "P1",
                "dettagli_report": {},
            }
    ...
    mocker.patch(
        "pages.admin.ia_view.learning_module.integrate_knowledge", return_value={"success": True}
    )
...
def test_render_access_logs_tab_with_data(mocker):
    ...
    df = pd.DataFrame(
        [{"timestamp": "2025-01-01 10:00:00", "username": "admin", "status": "success"}]
    )
