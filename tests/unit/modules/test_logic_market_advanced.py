...
def mock_st_shifts(mocker):
    ...
    return mocker.patch("src.modules.shifts.logic_market.st")
...
def test_rispondi_sostituzione_accettata(mocker, mock_st_shifts):
    ...
    mocker.patch("src.modules.shifts.logic_market.get_substitution_request_by_id", return_value=mock_req)
    mocker.patch("src.modules.shifts.logic_market.get_user_by_matricola", return_value={"Nome Cognome": "User"})
    mocker.patch("src.modules.shifts.logic_market.delete_substitution_request", return_value=True)
    mocker.patch("src.modules.shifts.logic_market.crea_notifica")
    
    # 2. Mock successo subentro
    mocker.patch("src.modules.shifts.logic_market.update_booking_user", return_value=True)
    mocker.patch("src.modules.shifts.logic_market.log_shift_change")
...
def test_rispondi_sostituzione_rifiutata(mocker, mock_st_shifts):
    ...
    mocker.patch("src.modules.shifts.logic_market.delete_substitution_request", return_value=True)
    mocker.patch("src.modules.shifts.logic_market.crea_notifica")
    mocker.patch("src.modules.shifts.logic_market.get_user_by_matricola", return_value=None)
...
def test_pubblica_bacheca_db_error(mocker, mock_st_shifts):
    ...
    mocker.patch("src.modules.shifts.logic_market.get_booking_by_user_and_shift", return_value={"ID_Prenotazione": "P1", "RuoloOccupato": "Tecnico"})
    
    # Simula errore sqlite3 nella transazione
    mock_conn = mocker.patch("src.modules.shifts.logic_market.get_db_connection")
...
def test_prendi_turno_non_disponibile(mocker, mock_st_shifts):
    ...
    mocker.patch("src.modules.shifts.logic_market.get_bacheca_item_by_id", return_value={"Stato": "Assegnato"})
