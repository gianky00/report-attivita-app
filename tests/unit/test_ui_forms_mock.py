...
class TestUIFormsMock(unittest.TestCase):
    ...
    def test_debriefing_form_render_and_submit(self):
        # Patch data manager
        with patch(
            "components.forms.debriefing_form.scrivi_o_aggiorna_risposta", return_value=True
        ) as mock_save:
    ...
    def test_relazione_oncall_form_render(self):
        with (
            patch("components.forms.relazione_oncall_form.get_all_users", return_value=MagicMock()),
            patch(
                "components.forms.relazione_oncall_form.get_report_knowledge_base_count",
                return_value=5,
            ),
        ):
    ...
    def test_shift_edit_form_render_success(self):
        ...
        mock_bookings.set_index.return_value.__getitem__.return_value.to_dict.return_value = {
            "U1": "Tecnico"
        }

        with (
            patch("components.forms.shift_edit_form.get_shift_by_id", return_value=mock_shift),
            patch(
                "components.forms.shift_edit_form.get_bookings_for_shift",
