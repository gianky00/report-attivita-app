...
def carica_dati_attivita_programmate():
    ...
    stato reale e lo storico dall'archivio, e restituisce un DataFrame unificato.
    """
    ...
    DESCRIZIONE_COL_INDEX = 6   # Colonna G
    ...
    status_map_excel = {
        'DA CHIUDERE': 'TERMINATA', 'EMESSO': 'IN CORSO', 'IN CORSO': 'IN CORSO',
        'INTERROTTO': 'SOSPESA', 'RICHIESTO': 'DA PROCESSARE'
    }
                ...
                for _, row in df_sheet.iterrows():
                    ...
                    if pd.notna(pdl) and str(pdl).strip():
                        ...
                        planned_activities.append({
                            ...
                            'Descrizione': row.iloc[DESCRIZIONE_COL_INDEX],
                            'StatoIniziale': status_map_excel.get(str(stato_excel_raw).strip().upper(), 'Pianificato'),
                            'GiorniProgrammati': ", ".join([g for g, i in GIORNI_COL_INDICES.items() if i < len(row) and str(row.iloc[i]).strip().upper() == 'X']) or "Non Programmato",
    ...
    if not planned_activities: return pd.DataFrame()

    df_planned = pd.DataFrame(planned_activities).drop_duplicates(subset='PdL', keep='first')

    # 2. Carica l'archivio degli stati e dello storico
    df_archived = carica_archivio_completo()
    if df_archived.empty:
        df_planned.rename(columns={'StatoIniziale': 'Stato'}, inplace=True)
        df_planned['Storico'] = [[] for _ in range(len(df_planned))]
        return df_planned
    ...
    # Raggruppa l'intero storico per ogni PdL
    df_history = df_archived.groupby('PdL').apply(lambda x: x.to_dict('records')).rename('Storico')

    # 3. Unisci le tre fonti di dati: pianificato, stato più recente, e storico completo
    df_merged = pd.merge(df_planned, df_latest_status[['PdL', 'Stato']], on='PdL', how='left')
    df_final = pd.merge(df_merged, df_history, on='PdL', how='left')

    # 4. Applica la gerarchia degli stati e riempi i dati mancanti
    df_final['Stato'].fillna(df_final['StatoIniziale'], inplace=True)
    df_final['Storico'].fillna(pd.Series([[] for _ in range(len(df_final))]), inplace=True)
    df_final.drop(columns=['StatoIniziale'], inplace=True)
