...
def render_guida_tab(ruolo: str) -> None:
    ...
    with st.expander("📋 Rendicontazione e PdL (Permessi di Lavoro)"):
        st.markdown(
            f"{render_svg_icon('report', 24)} **Il Ciclo di Vita di un Intervento**",
    ...
    with st.expander("📅 Turni, Reperibilità e Bacheca Scambi"):
        st.markdown(
            f"{render_svg_icon('calendar', 24)} **Pianificazione Team**", unsafe_allow_html=True
        )
        st.write("""
    ...
    with st.expander("📝 Relazioni di Fine Turno e Materiali"):
        st.markdown(
            f"{render_svg_icon('request', 24)} **Modulistica e Comunicazione**",
            unsafe_allow_html=True,
        )
        st.write("""
    ...
    with st.expander("🔍 Programmazione e Storico Interventi"):
        st.markdown(
            f"{render_svg_icon('archive', 24)} **Consultazione Dati**", unsafe_allow_html=True
        )
        st.write("""
