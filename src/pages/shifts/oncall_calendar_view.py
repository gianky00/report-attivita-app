...
def _render_oncall_edit_form(admin_id: str) -> None:
    ...
    with st.container(border=True):
        ...
        t2 = st.selectbox(
            "Tecnico 2:",
            options=u_list,
            format_func=lambda x: u_dict.get(x, x),
            index=1 if len(u_list) > 1 else 0,
        )

        c1, c2 = st.columns(2)
        if c1.button("Salva", type="primary", icon=ICONS["SAVE"]) and manual_override_logic(
            s_id, t1, t2, admin_id
        ):
            del st.session_state.editing_oncall_shift_id
            st.rerun()
        if c2.button("Annulla", icon=ICONS["CANCEL"]):
            del st.session_state.editing_oncall_shift_id
            st.rerun()
    st.stop()


def _render_oncall_management_form(df_contatti: pd.DataFrame) -> None:
    ...
    with st.container(border=True):
        ...
        if info:
            st.write(
                f"Turno di **{u_name}** del **{pd.to_datetime(info['Data']).strftime('%d/%m/%Y')}**"
            )
        else:
            st.error("Errore turno.", icon=ICONS["ERROR"])
            st.button("Indietro", on_click=lambda: st.session_state.pop("managing_oncall_shift_id"))
            return
        ...
        if st.session_state.get("oncall_swap_mode"):
            ...
            c1, c2 = st.columns(2)
            if c1.button(
                "Pubblica in Bacheca", icon=ICONS["BULLETIN"]
            ) and pubblica_turno_in_bacheca_logic(m_id, s_id):
                del st.session_state.managing_oncall_shift_id
                st.rerun()
            if c2.button("Chiedi Sostituzione", icon=ICONS["SWAP"]):
                st.session_state.oncall_swap_mode = True
                st.rerun()

        if st.button("Torna al Calendario", icon=":material/arrow_back:"):
            for k in (
                "managing_oncall_shift_id",
                "managing_oncall_user_matricola",
                "oncall_swap_mode",
            ):
...
def _render_oncall_swap_ui(m_id: str, s_id: str, df_contatti: pd.DataFrame) -> None:
    ...
    target = st.selectbox(
        "Collega:", valid["Matricola"].tolist(), format_func=lambda m: m_to_n.get(m, m)
    )

    if st.button(
        "Invia Richiesta", type="primary", icon=":material/send:"
    ) and richiedi_sostituzione_logic(m_id, target, s_id):
        for k in ("managing_oncall_shift_id", "oncall_swap_mode"):
            st.session_state.pop(k, None)
        st.rerun()


def _render_oncall_filters() -> None:
    ...
    if c[2].button("Vai al mese", use_container_width=True, icon=":material/event:"):
        ...
        st.rerun()
    if c[3].button("Sett. Corrente", use_container_width=True, icon=":material/today:"):
        st.session_state.week_start_date = today - datetime.timedelta(days=today.weekday())
        st.rerun()


def _render_oncall_export_section(df_p: pd.DataFrame, df_c: pd.DataFrame, ruolo: str) -> None:
    """Sezione export PDF per amministratori."""
    if ruolo == "Amministratore" and st.button(
        "Esporta PDF", use_container_width=True, icon=":material/picture_as_pdf:"
    ):
        st.info("Funzionalità di esportazione avviata...", icon=ICONS["INFO"])
        st.warning("L'export PDF richiede dati contestuali dei filtri.", icon=ICONS["WARNING"])


def _render_oncall_navigation() -> None:
    ...
    st.markdown(
        f"<h5 style='text-align: center;'>{start.day} {MESI_ITALIANI[start.month - 1][:3]} - {end.day} {MESI_ITALIANI[end.month - 1][:3]} {end.year}</h5>",
...
def _render_oncall_calendar_grid(
    df_p: pd.DataFrame, df_c: pd.DataFrame, matricola: str, ruolo: str
) -> None:
    ...
    m_to_n: dict[str, Any] = {
        str(k): v
        for k, v in pd.Series(df_c["Nome Cognome"].values, index=df_c["Matricola"].astype(str))
        .to_dict()
        .items()
    }
...
def _render_day_cell(
    ...
    if not shift.empty:
        ...
        if not p_today.empty:
            ...
            tech_html = "".join(
                [f"<div style='font-size: 0.8em; font-weight: 500;'>{s}</div>" for s in tech_list]
            )
        else:
