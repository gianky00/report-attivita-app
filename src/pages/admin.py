...
def render_technician_detail_view():
    """Visualizza i dettagli di performance di un tecnico (In Sviluppo)."""
...
def render_report_validation_tab(user_matricola):
    ...
    st.info(
        ...
        "- Puoi modificare il **Testo Report** e lo **Stato Attività** "
        "direttamente nella tabella.\n"
        "- Seleziona uno o più report e clicca 'Cancella' per rimuoverli.\n"
        "- Clicca 'Valida e Salva Modifiche' per processare i report, "
        "scriverli su Excel e rimuoverli da questa coda."
    )
        ...
        if not reports_to_delete_df.empty:
            if st.button(
                ...
                if delete_reports_by_ids(ids_to_delete):
                    st.success(
                        f"{len(ids_to_delete)} report sono stati cancellati."
                    )
                    st.rerun()
                else:
...
def _render_user_card(user):
    ...
    with st.container():
        ...
        with col1:
            ...
            st.markdown(
                f"**{user_name}** (`{user_matricola}`) - "
                f"*{user['Ruolo']}* - Stato: **{status}**"
            )
        with col2:
            st.button(
                "✏️ Modifica",
        ...
        if st.session_state.deleting_user_matricola == user_matricola:
            st.warning(
                f"Sei sicuro di voler eliminare l'utente **{user_name}**? "
                ...
                if new_nome and new_cognome and new_matricola:
                    if not df_contatti[
                        ...
                        if create_user(new_user_data):
                            st.success(
                                f"Utente '{nome_completo}' creato. "
                                "Dovrà impostare la password al primo accesso."
                            )
                            st.rerun()
                        else:
...
def render_access_logs_tab():
    ...
    st.info(
        "Questa sezione mostra tutti i tentativi di accesso registrati, "
...
def render_sistema_view():
    """Renderizza la vista 'Sistema'."""
    st.markdown('<div class="card">', unsafe_allow_html=True)
    tecnica_tabs = st.tabs(
    ...
    with tecnica_tabs[3]:
        ...
        with ia_sub_tabs[0]:
            ...
            pending_entries = [
                e
                for e in unreviewed_entries
                if e.get("stato") == "in attesa di revisione"
            ]
            if not pending_entries:
                st.success("🎉 Nessuna nuova voce da revisionare!")
            else:
                st.info(
                    f"Ci sono {len(pending_entries)} nuove voci da revisionare."
                )
            for i, entry in enumerate(pending_entries):
                title = (
                    f"**Voce ID:** `{entry['id']}` - "
                    f"**Attività:** {entry['attivita_collegata']}"
                )
                with st.expander(title, expanded=i == 0):
                    sugg_dt = datetime.datetime.fromisoformat(
                        entry["data_suggerimento"]
                    ).strftime("%d/%m/%Y %H:%M")
                    st.markdown(
                        f"*Suggerito da: **{entry['suggerito_da']}** il {sugg_dt}*"
                    )
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        new_equipment_key = st.text_input(
                            "Nuova Chiave Attrezzatura (es. 'motore_elettrico')",
                            key=f"key_{entry['id']}",
                        )
                        new_display_name = st.text_input(
                            "Nome Visualizzato (es. 'Motore Elettrico')",
                            key=f"disp_{entry['id']}",
                        )
                    with col2:
                        if st.button(
                            "✅ Integra nel Knowledge Core",
                            key=f"integrate_{entry['id']}",
                            type="primary",
                        ):
                            if new_equipment_key and new_display_name:
                                first_question = {
                                    ...
                                    "options": {
                                        k.lower().replace(" ", "_"): v
                                        for k, v in entry["dettagli_report"].items()
                                    },
                                }
                                details = {
                                ...
                                result = learning_module.integrate_knowledge(
                                    entry["id"], details
                                )
                                if result.get("success"):
                                    st.success(
                                        f"Voce '{entry['id']}' integrata!"
                                    )
                                    st.cache_data.clear()
                                    st.rerun()
                                else:
                                    st.error(
                                        f"Errore: {result.get('error')}"
                                    )
                            else:
                                st.warning(
                                    "Fornisci sia chiave che nome visualizzato."
                                )

        with ia_sub_tabs[1]:
            st.subheader("Gestione Modello IA")
            st.info(
                "Aggiorna la base di conoscenza dell'IA con le nuove relazioni. "
