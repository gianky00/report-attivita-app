...
def render_turni_list(
    df_turni, df_bookings, df_users, matricola_utente, ruolo, key_suffix
):
    if df_turni.empty:
        st.info("Nessun turno di questo tipo disponibile al momento.")
        return

    mostra_solo_disponibili = st.checkbox(
        "Mostra solo turni con posti disponibili", key=f"filter_turni_{key_suffix}"
    )
    matricola_to_name = pd.Series(
        df_users["Nome Cognome"].values, index=df_users["Matricola"].astype(str)
    ).to_dict()

    if ruolo == "Amministratore":
        search_term_turni = st.text_input(
            "Cerca per descrizione del turno...", key=f"search_turni_{key_suffix}"
        )
        if search_term_turni:
            df_turni = df_turni[
                df_turni["Descrizione"].str.contains(
                    search_term_turni, case=False, na=False
                )
            ]
    ...
    for _index, turno in df_turni.iterrows():
        ...
        tecnici_prenotati = len(
            prenotazioni_turno[prenotazioni_turno["RuoloOccupato"] == "Tecnico"]
        )
        aiutanti_prenotati = len(
            prenotazioni_turno[prenotazioni_turno["RuoloOccupato"] == "Aiutante"]
        )

        is_available = (tecnici_prenotati < posti_tecnico) or (
            aiutanti_prenotati < posti_aiutante
        )
        ...
        with st.container(border=True):
            ...
            data_str = pd.to_datetime(turno['Data']).strftime('%d/%m/%Y')
            st.caption(
                f"{data_str} | {turno['OrarioInizio']} - {turno['OrarioFine']}"
            )

            tech_icon = "✅" if tecnici_prenotati < posti_tecnico else "❌"
            aiut_icon = "✅" if aiutanti_prenotati < posti_aiutante else "❌"
            st.markdown(
                f"**Posti:** `Tecnici: {tecnici_prenotati}/{posti_tecnico}` "
                f"{tech_icon} | `Aiutanti: {aiutanti_prenotati}/{posti_aiutante}` "
                f"{aiut_icon}"
            )
            ...
            if not prenotazioni_turno.empty:
                ...
                for _, p in prenotazioni_turno.iterrows():
                    ...
                    nome_utente = matricola_to_name.get(
                        matricola, f"Matricola {matricola}"
                    )
                    st.markdown(f"- {nome_utente} (*{p['RuoloOccupato']}*)")

            st.markdown("---")

            prenotazione_utente = prenotazioni_turno[
                prenotazioni_turno["Matricola"] == str(matricola_utente)
            ]
            ...
            if not prenotazione_utente.empty:
                ...
                if col1.button(
                    "Cancella Prenotazione", key=f"del_{turno['ID_Turno']}_{key_suffix}"
                ):
                    if cancella_prenotazione_logic(matricola_utente, turno["ID_Turno"]):
                        st.rerun()
                if col2.button(
                    "📢 Pubblica in Bacheca",
                    key=f"pub_{turno['ID_Turno']}_{key_suffix}",
                ):
                    if pubblica_turno_in_bacheca_logic(
                        matricola_utente, turno["ID_Turno"]
                    ):
                        st.rerun()
                if col3.button(
                    "🔄 Chiedi Sostituzione",
                    key=f"ask_{turno['ID_Turno']}_{key_suffix}",
                ):
                    st.session_state["sostituzione_turno_id"] = turno["ID_Turno"]
                    st.rerun()
            else:
                opzioni = []
                if tecnici_prenotati < posti_tecnico:
                    opzioni.append("Tecnico")
                if aiutanti_prenotati < posti_aiutante:
                    opzioni.append("Aiutante")
                if opzioni:
                    ruolo_scelto = st.selectbox(
                        "Prenota come:",
                        opzioni,
                        key=f"sel_{turno['ID_Turno']}_{key_suffix}",
                    )
                    if st.button(
                        "Conferma Prenotazione",
                        key=f"add_{turno['ID_Turno']}_{key_suffix}",
                    ):
                        if prenota_turno_logic(
                            matricola_utente, turno["ID_Turno"], ruolo_scelto
                        ):
                            st.rerun()
                else:
            ...
            if st.session_state.get("sostituzione_turno_id") == turno["ID_Turno"]:
                ...
                ricevente_options = [
                    str(m)
                    for m in df_users["Matricola"]
                    if str(m) != str(matricola_utente)
                ]
                ricevente_matricola = st.selectbox(
                    "Seleziona collega:",
                    ricevente_options,
                    format_func=lambda m: matricola_to_name.get(m, m),
                    key=f"swap_select_{turno['ID_Turno']}_{key_suffix}",
                )

                if st.button(
                    "Invia Richiesta",
                    key=f"swap_confirm_{turno['ID_Turno']}_{key_suffix}",
                ):
                    if richiedi_sostituzione_logic(
                        matricola_utente, ricevente_matricola, turno["ID_Turno"]
                    ):
                        del st.session_state["sostituzione_turno_id"]
                        st.rerun()


def render_reperibilita_tab(
    df_prenotazioni, df_contatti, matricola_utente, ruolo_utente
):
    ...
    HOLIDAYS_2025 = [
        ...
        datetime.date(2025, 11, 1),
        datetime.date(2025, 12, 8),
        datetime.date(2025, 12, 13),  # Santa Lucia
        datetime.date(2025, 12, 25),
        datetime.date(2025, 12, 26),
    ]
    WEEKDAY_NAMES_IT = ["Lun", "Mar", "Mer", "Gio", "Ven", "Sab", "Dom"]
    ...
    if (
        "editing_oncall_shift_id" in st.session_state
        and st.session_state.editing_oncall_shift_id
    ):
        shift_id_to_edit = st.session_state.editing_oncall_shift_id
        turno_info = get_shift_by_id(shift_id_to_edit)
        with st.container(border=True):
            data_str = pd.to_datetime(turno_info['Data']).strftime('%d/%m/%Y')
            st.subheader(f"Modifica Assegnazione per il {data_str}")

            all_users = get_all_users()
            user_list = all_users["Matricola"].tolist()
            user_dict = pd.Series(
                all_users["Nome Cognome"].values, index=all_users["Matricola"]
            ).to_dict()

            new_tech1 = st.selectbox(
                "Seleziona Tecnico 1:",
                options=user_list,
                format_func=lambda x: user_dict.get(x, x),
            )
            new_tech2 = st.selectbox(
                "Seleziona Tecnico 2:",
                options=user_list,
                format_func=lambda x: user_dict.get(x, x),
                index=1 if len(user_list) > 1 else 0,
            )

            col1, col2 = st.columns(2)
            with col1:
                if st.button("Salva Modifiche", type="primary"):
                    if manual_override_logic(
                        shift_id_to_edit, new_tech1, new_tech2, matricola_utente
                    ):
    ...
    if (
        "managing_oncall_shift_id" in st.session_state
        and st.session_state.managing_oncall_shift_id
    ):
        ...
        user_to_manage_name = df_contatti[
        ...
        with st.container(border=True):
            ...
            if turno_info:
                data_str = pd.to_datetime(turno_info['Data']).strftime('%d/%m/%Y')
                st.write(
                    f"Stai modificando il turno di **{user_to_manage_name}** "
                    f"per il giorno **{data_str}**."
                )
            else:
                st.error("Dettagli del turno non trovati.")
                if st.button("⬅️ Torna al Calendario"):
                    if "managing_oncall_shift_id" in st.session_state:
            ...
            if st.session_state.get("oncall_swap_mode"):
                ...
                matricola_to_name = pd.Series(
                    contatti_validi["Nome Cognome"].values,
                    index=contatti_validi["Matricola"],
                ).to_dict()
                ricevente_matricola = st.selectbox(
                    "Seleziona collega:",
                    contatti_validi["Matricola"].tolist(),
                    format_func=lambda m: matricola_to_name.get(m, m),
                    key=f"swap_select_{shift_id_to_manage}",
                )

                c1, c2 = st.columns(2)
                if c1.button(
                    "Invia Richiesta",
                    key=f"swap_confirm_{shift_id_to_manage}",
                    type="primary",
                ):
                    if richiedi_sostituzione_logic(
                        matricola_to_manage, ricevente_matricola, shift_id_to_manage
                    ):
                        del st.session_state.managing_oncall_shift_id
                        if "oncall_swap_mode" in st.session_state:
                ...
                if col1.button("📢 Pubblica in Bacheca"):
                    if pubblica_turno_in_bacheca_logic(
                        matricola_to_manage, shift_id_to_manage
                    ):
                        del st.session_state.managing_oncall_shift_id
                        st.rerun()
            ...
            st.divider()
            if st.button(
                "⬅️ Torna al Calendario", key=f"cancel_manage_{shift_id_to_manage}"
            ):
                if "managing_oncall_shift_id" in st.session_state:
                    del st.session_state.managing_oncall_shift_id
                if "managing_oncall_user_matricola" in st.session_state:
                    del st.session_state.managing_oncall_user_matricola
                if "oncall_swap_mode" in st.session_state:
                    del st.session_state.oncall_swap_mode
                st.rerun()
        st.stop()
    ...
    if "week_start_date" not in st.session_state:
        st.session_state.week_start_date = today - datetime.timedelta(
            days=today.weekday()
        )
    ...
    with filter_cols[1]:
        selected_month = st.selectbox(
            "Mese", MESI_ITALIANI, index=today.month - 1, label_visibility="collapsed"
        )
    with filter_cols[2]:
        if st.button("Vai al mese", use_container_width=True):
            first_day_of_month = datetime.date(
                selected_year, MESI_ITALIANI.index(selected_month) + 1, 1
            )
            st.session_state.week_start_date = first_day_of_month - datetime.timedelta(
    ...
    if ruolo_utente == "Amministratore":
        if st.button("Esporta PDF", use_container_width=True):
            ...
            bookings_for_month = df_prenotazioni[
                df_prenotazioni["ID_Turno"].isin(monthly_shifts["ID_Turno"])
            ]
            ...
            if not bookings_for_month.empty:
                ...
                pdf_data = merged_data[
                    ["Data", "Nome Cognome", "RuoloOccupato"]
                ].to_dict("records")
                ...
                if pdf_path:
                    ...
                    subject = (
                        f"reperibilità strumentale ISAB SUD "
                        f"{month_name.lower()} {selected_year}"
                    )
                    body = (
                        f"In allegato si trasmette la reperibilità strumentale "
                        f"ISAB SUD per il mese di {month_name.lower()} {selected_year}."
                    )
                    ...
                    try:
                        ...
                        st.success("Bozza email creata in Outlook!")
                    ...
                    st.warning(
                        "Nessun dato di reperibilità da esportare."
                    )
            else:
    ...
    with nav_cols[1]:
        ...
        st.markdown(
            f"<h5 style='text-align: center; white-space: nowrap;'>"
            f"{start_date_str} - {end_date_str}</h5>",
    ...
    week_dates = [
        st.session_state.week_start_date + datetime.timedelta(days=i) for i in range(7)
    ]
    cols = st.columns(7)
    ...
    for i, day in enumerate(week_dates):
        with cols[i]:
            ...
            background_color = (
                "#e0f7fa" if is_today else ("#fff0f0" if is_special_day else "white")
            )
            ...
            if not shift_today.empty:
                ...
                prenotazioni_today = df_prenotazioni[
                    df_prenotazioni["ID_Turno"] == shift_id_today
                ]
                matricola_to_name = pd.Series(
                    df_contatti["Nome Cognome"].values,
                    index=df_contatti["Matricola"].astype(str),
                ).to_dict()
                ...
                if not prenotazioni_today.empty:
                    ...
                    for _, booking in prenotazioni_today.iterrows():
                        ...
                        technician_name = matricola_to_name.get(
                            technician_matricola, f"Matricola {technician_matricola}"
                        )
                    ...
                    if tech_display_list:
                        managed_user_matricola = str(
                            prenotazioni_today.iloc[0]["Matricola"]
                        )
                    technicians_html = "".join(
                        [
                            f"<div style='font-size: 0.9em; font-weight: 500; "
                            f"line-height: 1.3; margin-bottom: 2px;'>{s}</div>"
                            for s in tech_display_list
                        ]
                    )
                else:
                    technicians_html = (
                        "<span style='color: grey; font-style: italic;'>Libero</span>"
                    )
            else:
                technicians_html = (
                    "<span style='color: grey; font-style: italic;'>N/D</span>"
                )

            st.markdown(
                f"""
                <div style="border: {border_style}; border-radius: 8px; padding: 8px;
                background-color: {background_color}; height: 140px; display: flex;
                flex-direction: column; justify-content: space-between;">
                    <div style="display: flex; justify-content: space-between;
                    align-items: flex-start; width: 100%;">
                        <div style="text-align: left;">
                            <p style="font-weight: bold; color: {day_color}; margin: 0;
                            font-size: 0.9em;">{WEEKDAY_NAMES_IT[day.weekday()]}</p>
            ...
            can_manage = (
                user_is_on_call or ruolo_utente == "Amministratore"
            ) and shift_id_today
            if can_manage:
                action_cols = st.columns(2)
                with action_cols[0]:
                    if st.button(
                        "Gestisci", key=f"manage_{day}", use_container_width=True
                    ):
                        st.session_state.managing_oncall_shift_id = shift_id_today
                        st.session_state.managing_oncall_user_matricola = (
                            managed_user_matricola
                        )
                        st.rerun()
                with action_cols[1]:
                    if ruolo_utente == "Amministratore":
                        if st.button(
                            "Modifica", key=f"edit_{day}", use_container_width=True
                        ):
...
def render_gestione_turni_tab(matricola_utente, ruolo):
    ...
    turni_disponibili_tab, bacheca_tab, sostituzioni_tab = st.tabs(
        ["📅 Turni", "📢 Bacheca", "🔄 Sostituzioni"]
    )

    with turni_disponibili_tab:
        assistenza_tab, straordinario_tab, reperibilita_tab = st.tabs(
            ["Turni Assistenza", "Turni Straordinario", "Turni Reperibilità"]
        )
        with assistenza_tab:
    ...
    with bacheca_tab:
        ...
        turni_disponibili_bacheca = df_bacheca[
            df_bacheca["Stato"] == "Disponibile"
        ].sort_values(by="Timestamp_Pubblicazione", ascending=False)
        if turni_disponibili_bacheca.empty:
            ...
            for _, bacheca_entry in turni_disponibili_bacheca.iterrows():
                ...
                if not turno_details:
                    st.warning(
                        f"Dettagli non trovati per il turno ID "
                        f"{bacheca_entry['ID_Turno']}. "
                        "Potrebbe essere stato rimosso."
                    )
                    continue

                matricola_originale = str(bacheca_entry["Tecnico_Originale_Matricola"])
                nome_originale = matricola_to_name.get(
                    matricola_originale, f"Matricola {matricola_originale}"
                )
                with st.container(border=True):
                    st.markdown(
                        f"**{turno_details['Descrizione']}** "
                        f"({bacheca_entry['Ruolo_Originale']})"
                    )
                    data_str = pd.to_datetime(turno_details['Data']).strftime('%d/%m/%Y')
                    st.caption(
                        f"Data: {data_str} | Orario: {turno_details['OrarioInizio']} - "
                        f"{turno_details['OrarioFine']}"
                    )
                    pub_dt = pd.to_datetime(bacheca_entry['Timestamp_Pubblicazione'])
                    st.write(
                        f"Pubblicato da: {nome_originale} il "
                        f"{pub_dt.strftime('%d/%m %H:%M')}"
                    )
                    ruolo_richiesto = bacheca_entry["Ruolo_Originale"]
                    is_eligible = not (
                        ruolo_richiesto == "Tecnico" and ruolo == "Aiutante"
                    )
                    if is_eligible:
                        if st.button(
                            "Prendi questo turno",
                            key=f"take_{bacheca_entry['ID_Bacheca']}",
                        ):
                            if prendi_turno_da_bacheca_logic(
                                matricola_utente, ruolo, bacheca_entry["ID_Bacheca"]
                            ):
                                st.rerun()
                    else:
    ...
    with sostituzioni_tab:
        ...
        richieste_ricevute = df_substitutions[
            df_substitutions["Ricevente_Matricola"] == str(matricola_utente)
        ]
        ...
        for _, richiesta in richieste_ricevute.iterrows():
            with st.container(border=True):
                richiedente_nome = matricola_to_name.get(
                    str(richiesta["Richiedente_Matricola"]), "Sconosciuto"
                )
                st.markdown(
                    f"**{richiedente_nome}** ti ha chiesto un cambio "
                    f"per il turno **{richiesta['ID_Turno']}**."
                )
                c1, c2 = st.columns(2)
                if c1.button("✅ Accetta", key=f"acc_{richiesta['ID_Richiesta']}"):
                    if rispondi_sostituzione_logic(
                        richiesta["ID_Richiesta"], matricola_utente, True
                    ):
                        st.rerun()
                if c2.button("❌ Rifiuta", key=f"rif_{richiesta['ID_Richiesta']}"):
                    if rispondi_sostituzione_logic(
                        richiesta["ID_Richiesta"], matricola_utente, False
                    ):
                        st.rerun()
        st.divider()
        st.markdown("#### 📤 Richieste Inviate")
        richieste_inviate = df_substitutions[
            df_substitutions["Richiedente_Matricola"] == str(matricola_utente)
        ]
        if richieste_inviate.empty:
            st.info("Nessuna richiesta di sostituzione inviata.")
        for _, richiesta in richieste_inviate.iterrows():
            ricevente_nome = matricola_to_name.get(
                str(richiesta["Ricevente_Matricola"]), "Sconosciuto"
            )
            st.markdown(
                f"- Richiesta inviata a **{ricevente_nome}** per il turno "
                f"**{richiesta['ID_Turno']}**."
            )
