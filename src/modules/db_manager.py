"""
Interfaccia di gestione del database SQLite.
Fornisce funzioni CRUD per turni, prenotazioni, log, report e utenti.
"""
...
# Il database si trova nella root del progetto
...
def get_db_connection() -> sqlite3.Connection:
    ...
    Configura row_factory per permettere l'accesso ai risultati tramite nome colonna.
    """
...
def get_notifications_for_user(matricola: str) -> pd.DataFrame:
    ...
    try:
        query = (
            "SELECT * FROM notifiche WHERE Destinatario_Matricola = ? "
...
def update_booking_user(shift_id: str, old_matricola: str, new_matricola: str) -> bool:
    ...
    try:
        with conn:
            sql = (
                "UPDATE prenotazioni SET Matricola = ? "
                "WHERE ID_Turno = ? AND Matricola = ?"
            )
...
def delete_bookings_for_shift(shift_id: str, cursor=None) -> bool:
    """Cancella tutte le prenotazioni per un turno specifico."""
    conn = None
    try:
        if cursor is None:
            conn = get_db_connection()
        ...
        print(
            f"Errore durante la cancellazione delle prenotazioni "
            f"per il turno {shift_id}: {e}"
        )
        return False
    finally:
        if conn:
...
def count_unread_notifications(matricola: str) -> int:
    ...
    try:
        ...
        query = (
            ...
            "WHERE Destinatario_Matricola = ? AND Stato = 'Non letta'"
        )
...
def process_and_commit_validated_relazioni(
    validated_df: pd.DataFrame, validator_id: str
) -> bool:
    ...
    try:
        with conn:
            ...
            for _, row in validated_df.iterrows():
                ...
                try:
                    data_intervento_iso = pd.to_datetime(
                        row["data_intervento"], format="%d/%m/%Y"
                    ).isoformat()
                except ValueError:
                    data_intervento_iso = row[
                        "data_intervento"
                    ]  # Mantieni il formato se non è valido
...
def get_unvalidated_relazioni() -> pd.DataFrame:
    ...
    try:
        query = (
            "SELECT * FROM relazioni WHERE stato = 'Inviata' "
            "ORDER BY timestamp_invio ASC"
        )
...
def delete_reports_by_ids(report_ids: list) -> bool:
    ...
    try:
        with conn:
            ...
            query = (
                f"DELETE FROM report_da_validare WHERE id_report IN ({placeholders})"
            )
...
def process_and_commit_validated_reports(validated_data: list) -> bool:
    ...
    for report_dict in validated_data:
        ...
        if not salva_report_intervento(report_data):
            # Logga l'errore ma non bloccare la validazione degli altri report
            print(
                f"ATTENZIONE: Report {report_dict.get('id_report')} "
                "non salvato nel DB storico 'report_interventi'."
            )
    ...
    if not delete_reports_by_ids(report_ids_to_delete):
        print(
            "ERRORE CRITICO: I report sono stati salvati nel DB storico, "
            "ma non è stato possibile rimuoverli dalla coda di validazione."
        )
        return False
...
def get_validated_reports(table_name: str) -> pd.DataFrame:
    ...
    conn = get_db_connection()
    try:
        # Costruisce la query in modo sicuro
        if table_name not in [
            "relazioni",
            "report_validati",
        ]:  # Whitelist delle tabelle consentite
            raise ValueError("Nome tabella non valido")

        query = (
            f"SELECT * FROM {table_name} WHERE stato = 'Validata' "
...
def salva_storico_materiali(dati_richiesta: dict) -> bool:
    ...
    try:
        with conn:
            ...
            sql = (
                f"INSERT INTO storico_richieste_materiali ({cols}) "
        ...
        print(
            f"Errore durante il salvataggio della richiesta materiali "
            f"nello storico: {e}"
        )
        return False
    finally:
...
def salva_storico_assenze(dati_richiesta: dict) -> bool:
    ...
    try:
        with conn:
            ...
            placeholders = ", ".join("?" for _ in dati_richiesta)
            sql = (
                f"INSERT INTO storico_richieste_assenze ({cols}) "
                f"VALUES ({placeholders})"
            )
            cursor.execute(sql, list(dati_richiesta.values()))
        return True
        ...
        print(
            f"Errore durante il salvataggio della richiesta assenze nello storico: {e}"
        )
        return False
    finally:
        if conn:
...
def get_storico_richieste_materiali() -> pd.DataFrame:
    ...
    try:
        query = (
            "SELECT * FROM storico_richieste_materiali "
            "ORDER BY timestamp_approvazione DESC"
        )
        df = pd.read_sql_query(query, conn)
        return df
    except (sqlite3.Error) as e:
...
def get_storico_richieste_assenze() -> pd.DataFrame:
    ...
    try:
        query = (
            "SELECT * FROM storico_richieste_assenze "
            "ORDER BY timestamp_approvazione DESC"
        )
        df = pd.read_sql_query(query, conn)
        return df
...
def get_validated_intervention_reports(
    ...
    try:
        if matricola_tecnico:
            query = (
                "SELECT * FROM report_interventi "
                "WHERE matricola_tecnico = ? "
...
def delete_report_by_id(report_id: str, table_name: str) -> bool:
    ...
    try:
        ...
        print(
            f"Errore durante la cancellazione del report {report_id} "
            f"da {table_name}: {e}"
        )
        return False
    finally:
...
def move_report_atomically(
    report_data: dict, source_table: str, dest_table: str
) -> bool:
    """Sposta un report da una tabella all'altra in modo atomico."""
    conn = get_db_connection()
    try:
        if not all(
            isinstance(name, str) and (name.isalnum() or "_" in name)
...
def get_unvalidated_reports_by_technician(matricola_tecnico: str) -> pd.DataFrame:
    ...
    try:
        query = (
            "SELECT * FROM report_da_validare WHERE matricola_tecnico = ? "
        ...
        st.error(
            f"Errore nel caricamento dei report non validati "
            f"per il tecnico {matricola_tecnico}: {e}"
        )
        return pd.DataFrame()
    finally:
        if conn:
