...
def richiedi_sostituzione_logic(
    matricola_richiedente: str, matricola_ricevente: str, turno_id: str
) -> bool:
    ...
    new_request_data = {
        ...
        "ID_Turno": turno_id,
        "Richiedente_Matricola": matricola_richiedente,
        "Ricevente_Matricola": matricola_ricevente,
...
def rispondi_sostituzione_logic(
    id_richiesta: str, matricola_che_risponde: str, accettata: bool
) -> bool:
    ...
    msg = f"{nome_che_risponde} ha {esito} la tua richiesta di cambio per il turno {turno_id}."
    ...
    try:
        ...
        with conn:
            ...
            new_bacheca_item = {
                ...
                "ID_Turno": turno_id,
                "Tecnico_Originale_Matricola": matricola_richiedente,
        ...
        if turno_info:
            ...
            msg = f"{ICONS['BULLETIN']} Turno libero: '{turno_info['Descrizione']}' del {data_str} ({booking_to_publish['RuoloOccupato']})."
            all_users = get_all_users()
            for _, user in all_users.iterrows():
                if str(user["Matricola"]) != matricola_richiedente:
...
def prendi_turno_da_bacheca_logic(
    matricola_subentrante: str, ruolo_utente: str, id_bacheca: str
) -> bool:
