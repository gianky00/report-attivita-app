...
def render_sidebar(matricola_utente: str, nome_utente_autenticato: str, ruolo: str) -> None:
    ...
    with st.sidebar:
        ...
        st.markdown(
            f"<h2 style='font-size: 1.5rem; margin-bottom: 0;'>Benvenuto, <span style='color: #4364F7;'>{nome_utente_autenticato.split()[0]}</span></h2>",
            unsafe_allow_html=True,
        )
        st.markdown(
            f"<p style='color: #64748b; font-size: 0.9rem; margin-top: 0;'>{ruolo}</p>",
            unsafe_allow_html=True,
        )
        ...
        if st.button(
            "Esci dal portale", icon=ICONS["LOGOUT"], use_container_width=True, key="nav_logout"
        ):
        ...
        st.markdown(
            f"<div style='text-align: center; color: #94a3b8; font-size: 0.7rem; margin-top: 1rem;'>HORIZON PLATFORM v{APP_VERSION}</div>",
            unsafe_allow_html=True,
        )
...
def _render_nav_buttons() -> None:
    ...
    if st.button(
        "Attività Assegnate", icon=ICONS["ATTIVITA"], use_container_width=True, key="nav_tasks"
    ):
    ...
    if st.button("Gestione Turni", icon=ICONS["TURNI"], use_container_width=True, key="nav_shifts"):
        ...
        st.rerun()
    if st.button(
        "Richieste", icon=ICONS["RICHIESTE"], use_container_width=True, key="nav_requests"
    ):
...
def _render_settings_menu() -> None:
    ...
    if st.button(
        "Impostazioni", icon=ICONS["ADMIN"], use_container_width=True, key="nav_settings_toggle"
    ):
        st.session_state.expanded_menu = "Impostazioni" if not is_expanded else ""
        st.rerun()

    if is_expanded and st.button(
        "Generali", icon=ICONS["SECURITY"], use_container_width=True, key="nav_settings_gen"
    ):
...
def _render_admin_menu() -> None:
    ...
    if st.button(
        "Amministrazione", icon=ICONS["ADMIN"], use_container_width=True, key="nav_admin_toggle"
    ):
